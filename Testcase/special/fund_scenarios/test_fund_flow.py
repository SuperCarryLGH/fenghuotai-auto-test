...
# ============================================================
COMPANY_ID = 5
SORTING_CENTER_ID = 2075766704452820993
STATION_ID = "2071933798574993409"
TEST_USER_MOBILE = "15617617160"
TEST_USER_ID = 2057279533119180801
TEST_ADDRESS_ID = "2057279533119180801"
...
def _get_sorting_center_wallet(headers: dict) -> dict:
    ...
    for item in items:
        if str(item.get("id")) == str(SORTING_CENTER_ID):
            return item
    for item in items:
        if str(item.get("walletId")) == str(SORTING_CENTER_ID):
            return item
    return items[0] if items else None
...
class TestFundFlowScenarios:
    ...
    def test_all_scenarios(self, api_session):
        ...
        after_s15 = _manual_step(after_s14, "S15后",
            "网点: 1 (18600000000) | 垫付给用户: 15617637160 | 金额: 10元",
            {"公司总": 0, "公司微信": 0, "公司支付宝": 0,
             "待分配": 0, "已分配": 0,
             "分拣中心总": 0, "分拣中心微信": 0, "分拣中心支付宝": 0,
