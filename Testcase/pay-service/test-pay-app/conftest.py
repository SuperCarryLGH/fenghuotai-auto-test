...
def autotest_app_id(api_session, auth_headers):
    ...
    assert data["code"] == 0
    rec_id = data["data"]
    print(f"[Fixture] created autotest_app_id = {rec_id}")

    yield rec_id

    api_session.delete(f"{ADMIN_URL}/admin-api/pay/app/delete", params={"id": rec_id}, headers=auth_headers)
    print(f"[Fixture] deleted autotest_app_id = {rec_id}")
