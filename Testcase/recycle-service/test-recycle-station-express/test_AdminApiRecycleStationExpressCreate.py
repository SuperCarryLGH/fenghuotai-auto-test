...
class Test_AdminApiRecycleStationExpressCreate:
    ...
    def _cleanup(self, api_session, auth_headers):
        ...
        if self._created_id is not None:
            try:
                api_session.delete(f"{ADMIN_URL}/admin-api/recycle/station/express/delete", params={"id": self._created_id}, headers=auth_headers)
            except Exception as e:
                print(f"[cleanup] 删除失败 {self._created_id}: {e}")


    @pytest.mark.smoke
    def test_AdminApiRecycleStationExpressCreate(self, api_session, auth_headers, ok):
