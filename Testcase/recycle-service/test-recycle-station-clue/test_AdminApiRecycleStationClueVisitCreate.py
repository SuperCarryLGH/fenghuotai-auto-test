...
class Test_AdminApiRecycleStationClueVisitCreate:
    ...
    def _cleanup(self, api_session, auth_headers):
        ...
        if self._created_id is not None:
            try:
                api_session.delete(f"{ADMIN_URL}/admin-api/recycle/station-clue/visit-delete", params={"id": self._created_id}, headers=auth_headers)
            except Exception as e:
                print(f"[cleanup] 删除失败 {self._created_id}: {e}")

    @pytest.mark.smoke
    @pytest.mark.skip(reason="线索/签约业务流字段契约未完全确认(网点ID/拜访意向/运行状态等)，待补")
    def test_AdminApiRecycleStationClueVisitCreate(self, clue_chain, api_session, ok):
        chain, clue_id, clue_no, auth_headers = clue_chain
        url = f"{ADMIN_URL}/admin-api/recycle/station-clue/visit-create"
        r = ok(api_session.post(url, json={"clueId": clue_id}, headers=auth_headers))
