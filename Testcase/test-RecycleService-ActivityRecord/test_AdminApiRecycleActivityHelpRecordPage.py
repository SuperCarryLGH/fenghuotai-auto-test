import random
import pytest
from config import ADMIN_URL
from Common.loader import load_common, load_page,load_recycle_activity

common = load_common()
page = load_page()
report_msg = load_recycle_activity()


class Test_AdminApiRecycleActivityRecordPage:
    """获得活动参与记录分页"""

    @pytest.mark.smoke
    def test_AdminApiRecycleActivityRecordPage(self, api_session, auth_headers):
        url = f"{ADMIN_URL}/admin-api/recycle/activity-record/page"
        body ={
            "pageNo": page["page"]['pageNo'],
            "pageSize": page["page"]['pageSize'],
            "activityId": report_msg["report_msg"]['activityId'],
        }
        resp = api_session.get(url, params=body, headers=auth_headers)
        assert resp.status_code == 200
        r = resp.json()
        assert r["code"] == 0
        print(r)
