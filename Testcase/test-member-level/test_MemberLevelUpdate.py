import pytest
from config import ADMIN_URL
from Common.loader import load_member_level_update
update = load_member_level_update()

class TestMemberLevelUpdate:
    """更新会员等级"""

    @pytest.mark.smoke
    def test_MemberLeveUpdate(self, api_session,auth_headers):
        """
        运行: TEST_ENV=dev USE_MOCK=false pytest ... -v -s
        """
        url = f"{ADMIN_URL}/admin-api/member/level/update"
        params = {
            "name": update["update"]["name"],
            "experience": update["update"]["experience"],
            "level": update["update"]["level"],
            "discountPercent": update["update"]["discountPercent"],
            "icon": update["update"]["icon"],
            "backgroundUrl": update["update"]["backgroundUrl"],
            "status": update["update"]["status"],
            "id": update["update"]["id"]
            }

        resp = api_session.put(url, headers=auth_headers,json=params)
        assert resp.status_code == 200
        r = resp.json()
        assert r["code"] == 0
        #assert r["data"] == {}
        print(r)








#test_AppApiCooperationGetByPlatform